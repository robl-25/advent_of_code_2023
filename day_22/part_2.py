...
class Brick:
    ...
    def cascade_fall(self, fallen_bricks):
        fallen_bricks.add(self)

        cascading_bricks = [b for b in self.supports if not b.supported_by - fallen_bricks]

        return len(cascading_bricks) + sum(b.cascade_fall(fallen_bricks) for b in cascading_bricks)
...
print(sum(b.cascade_fall(set()) for b in bricks))
